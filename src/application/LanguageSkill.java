...
public class LanguageSkill extends UIControl{	
	...
	public void makeTracker(HBox sOC, HBox sTC, HBox sThC, Label sOL, Label sTL, Label sThL, ProgressBar bO, 
		...
		for (String element : getPickedList()) {			 
			...
			if((element.equals("french") || element.equals("english") ||  element.equals("arabic")) && counter == 0) {  //skill One. Method difference from parent and child class is because of this line.(Polymorphism)		 
				this.sOL = element;
			...
			else if((element.equals("french") || element.equals("english") ||  element.equals("arabic")) && counter == 1) { //skill Two. Method difference from parent and child class is because of this line.(Polymorphism)
				this.sTL=element;
				sTC.getChildren().addAll(sTL,bT,iT, tT, mT);
				changeLabel(sTL," "+ element+" ");
				tC.getChildren().add(sTC);
			}
			else if((element.equals("french") || element.equals("english") ||  element.equals("arabic")) && counter == 2) { //skillThree. Method difference from parent and child class is because of this line.(Polymorphism)
				this.sThL=element;
		...
		if(progressOne <1) { // if progress is not finished continue earning or losing XP.
			...
			if(errorOne==false) {
				...
				if (textValueOne ==0 && progressOne>0) { //If some progress was made, but no more practicing is done, lose XP.
					XP = 300; // Difference among other classes is the XP loss.(polymorphism)
	...
	public double setProgressXP(int textValue) {				
		...
		return (textValue/300.0); // Difference among the other classes is the denominator.
	}
}
