...
public class MusicSkill extends UIControl{
	...
	public void makeTracker(HBox sOC, HBox sTC, HBox sThC, Label sOL, Label sTL, Label sThL, ProgressBar bO, 
		...
		for (String element : getPickedList()) {			 
			...
			if((element.equals("guitar") || element.equals("piano") ||  element.equals("drums")) && counter == 0) {  //skill One. Method difference from parent and child class is because of this line.(Polymorphism)			 
				this.sOL = element;
			...
			else if((element.equals("guitar") || element.equals("piano") ||  element.equals("drums")) && counter == 1) { //skill Two. Method difference from parent and child class is because of this line.(Polymorphism)
				this.sTL=element;
				sTC.getChildren().addAll(sTL,bT,iT, tT, mT);
				changeLabel(sTL," "+ element+" ");
				tC.getChildren().add(sTC);
			}
			else if((element.equals("guitar") || element.equals("piano") ||  element.equals("drums")) && counter == 2) { //skillThree. Method difference from parent and child class is because of this line.(Polymorphism)
				this.sThL=element;
		...
		if(progressOne <1) { // if progress is not finished continue earning or losing XP.
			...
			if(errorOne==false) {
				...
				if (textValueOne ==0 && progressOne>0) { //If some progress was made, but no more practicing is done, lose XP.
					XP = 180; // Difference among other classes is the XP loss.(polymorphism)
		...
		if (sOL!=null && sTL!=null && sThL!=null) {  // If all three sub-skills are picked by user.
			if((textValueOne + textValueTwo + textValueThree) <=24 && errorOne==false && errorTwo==false && errorThree==false ) { // Makes sure that the sum of the inputs is less than or equal to 24 hours.
				...
				if (textValueOne==0 && progressOne > 0 && progressOne<1 && textStringOne!="" && textStringTwo!="" && textStringThree!="" ) {
					progressOne -= .027; //decrementing progress for not practicing;
				...
				if (textValueTwo==0 && progressTwo>0 && progressTwo<1 && textStringOne!="" && textStringTwo!="" && textStringThree!="" )  {
					progressTwo -= .027; //decrementing progress for not practicing;
				...
				if (textValueThree==0 && progressThree>0 && progressThree<1 && textStringOne!="" && textStringTwo!="" && textStringThree!="" ) {
					progressThree -= .027; //decrementing progress for not practicing;
			...
			if((textValueOne) <=24 && errorOne==false && errorTwo==false && errorThree==false ) { // Makes sure that the sum of the inputs is less than or equal to 24 hours.
				...
				if (progressOne>=1) { // If all progresses are finished.
	...
	/**
	 * Converts text value given by user to XP points polymorphically with different values.
	...
	public double setProgressXP(int textValue) {				
		...
		return (textValue/230.0); // Difference among the other classes is the denominator.
	}
